...
def prepare_file(source_path, target_path, target_duration, file_type):
    ...
    # Trim to target duration if specified (but don't pad to avoid silence)
    if target_duration is not None:
        target_samples = int(target_duration * TARGET_SR)
        current_samples = len(audio)
        actual_duration = current_samples / TARGET_SR

        if current_samples > target_samples:
            audio = audio[:target_samples]
            print(f"    ✓ Trimmed to {target_duration*1000:.0f} ms")
        elif current_samples < target_samples:
            # Don't pad - use actual duration to avoid adding silence at the end
            print(f"    ⚠ File is {actual_duration*1000:.0f} ms (target: {target_duration*1000:.0f} ms)")
            print(f"    ✓ Keeping actual duration (no padding to avoid silence)")
        else:
            print(f"    ✓ Duration is correct ({target_duration*1000:.0f} ms)")
    else:
        print(f"    ✓ Keeping full duration ({len(audio) / TARGET_SR * 1000:.0f} ms)")
...
if full_100 and stem_100:
    print("\n" + "="*70)
    print("PROCESSING: 100-WINDOW VERSION (4700 ms)")
    print("="*70)
    ...
    print("\nFor 100-window version (4700 ms):")
