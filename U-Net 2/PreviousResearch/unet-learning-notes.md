...
## NumPy to Librosa Migration (2025-11-05)

### The Problem: Strategy 3 (dB-Space EQ) Was Broken

**Symptom:** RuntimeWarning and magnitude explosion when using dB-space optimization

```python
# Original numpy implementation (BROKEN):
Pass 2/5: slice_2_vertical
RuntimeWarning: invalid value encountered in log10
  vocal_fp_db = 20 * np.log10(vocal_fp + 1e-8)  # Creates NaN!

✓ Max magnitude: 274.48   # Pass 1
✓ Max magnitude: 1,282,745 # Pass 2 - EXPLOSION!
```

**Root cause:** Manual dB conversion with `20 * np.log10(x + 1e-8)` doesn't handle edge cases properly:
- When `vocal_fp + 1e-8` produces certain values, log10() creates NaN
- NaN propagates through gradient calculations
- EQ curves become invalid, causing magnitude explosion
- Audio output is completely destroyed

### The Solution: librosa.amplitude_to_db()

**After replacing with librosa:**
```python
# New librosa implementation (WORKS!):
vocal_fp_db = librosa.amplitude_to_db(vocal_fp, ref=1.0, amin=1e-8)

✓ Max magnitude: 274.48   # Pass 1
✓ Max magnitude: 267.89   # Pass 2 - stable!
```

**Result:** "Oh wow, now strategy3_dbspace sounds great!" - User feedback

**Why it works:**
- `librosa.amplitude_to_db()` clips values to `amin` **before** applying log
- Proper handling of zero/near-zero amplitudes
- Audio-aware reference level handling
- Numerically stable for all input ranges
- No RuntimeWarnings, no NaN propagation

### Complete NumPy → Librosa Replacements Made

#### 1. Magnitude/Phase Extraction
```python
# Before (numpy):
...
```
**Why:** `magphase()` returns phase as complex phasor (not angle), handles zero-magnitude bins safely

#### 2. Complex STFT Reconstruction
```python
# Before (numpy):
reconstructed_stft = magnitude * np.exp(1j * phase)

# After (librosa):
reconstructed_stft = magnitude * phase  # phase is already complex phasor
```
**Why:** No manual exponential needed, phase accumulation handled correctly

#### 3. dB Conversion (CRITICAL FIX)
```python
# Before (numpy - BROKEN):
with np.errstate(divide='ignore', invalid='ignore'):
    working_mag_db = 20 * np.log10(working_mag + 1e-8)  # Creates NaN!

# After (librosa - WORKS):
working_mag_db = librosa.amplitude_to_db(working_mag, ref=1.0, amin=1e-8)
```
**Why:** Proper clipping before log, no NaN creation, no warning suppression needed

#### 4. Inverse dB Conversion
```python
# Before (numpy):
refined_mag = 10 ** (refined_mag_db / 20)

# After (librosa):
refined_mag = librosa.db_to_amplitude(refined_mag_db, ref=1.0)
```
**Why:** Matches the forward conversion, consistent reference handling

#### 5. Final Normalization (Already Using)
```python
# We were already using manual normalization correctly:
refined_mag = refined_mag / np.max(refined_mag) * original_max
```
**Note:** User tested `librosa.util.normalize()` but manual approach sounded best, so we kept it.

### What We Kept (NumPy Operations)

These have no librosa alternatives and work correctly:
- Array creation: `np.zeros()`, `np.ones()`, `np.array()`, `np.zeros_like()`
- Math operations: `np.mean()`, `np.clip()`
- Interpolation: `np.interp()`, `np.linspace()`
- Data cleaning: `np.nan_to_num()`
- 2D convolution: `scipy.ndimage.convolve()` (no librosa alternative)

### Key Lessons Learned

1. **Always use librosa for dB conversions** - `librosa.amplitude_to_db()` handles edge cases that manual `log10()` doesn't
2. **librosa.magphase() prevents NaN** - Safer than `np.abs()` + `np.angle()` for zero bins
3. **Don't suppress warnings** - The `np.errstate()` was hiding the real problem (NaN creation)
4. **Test strategies side-by-side** - Created test_gain_strategies.py comparing 4 approaches
5. **Normalization matters** - Manual global normalization (`/ max * original_max`) sounds best for our use case
6. **Not everything needs librosa** - Generic array ops should stay numpy

### Files Updated

- `test_gain_strategies.py` - Updated with all librosa replacements, Strategy 3 now works!
- `numpy_to_librosa_audit.md` - Complete audit of 58 np.* function calls with recommendations
- `test_normalization_strategies.py` - Tests 4 normalization approaches (manual sounds best)

### Before/After Audio Quality

**Strategy 3 (dB-Space EQ):**
- **Before:** Completely broken, magnitude explosion to 1.2M, unusable output
- **After:** "Sounds great!" - stable optimization, clean vocal separation

**Impact:** The dB-space approach is now viable! Working in dB domain can be more intuitive for audio (additive vs multiplicative), and now it's numerically stable.
