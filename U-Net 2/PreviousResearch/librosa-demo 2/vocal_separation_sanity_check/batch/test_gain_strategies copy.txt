...
def window_to_bottleneck(window, sr):
    """Extract 400-point frequency profile from window"""
    with np.errstate(divide='ignore', invalid='ignore'):
        ...
        bottleneck_vector = downsample_spectrum(layer1, factor=2)

        # Core: 400-point frequency profile
        freq_profile_400 = np.interp(
            x=np.linspace(0, sr/2, 400),
            xp=np.linspace(0, sr/2, len(layer1)),
            fp=layer1
        )
...
def optimize_strategy3_db_space(slice_name, vocal_fps, mixture_mag, num_windows, sr, current_mag=None):
    ...
    # Convert to dB
    working_mag_db = 20 * np.log10(working_mag + 1e-8)
    ...
    for iteration in range(CONFIG['num_iterations']):
        ...
        for win_idx in range(num_windows):
            ...
            vocal_fp_db = 20 * np.log10(vocal_fp + 1e-8)
