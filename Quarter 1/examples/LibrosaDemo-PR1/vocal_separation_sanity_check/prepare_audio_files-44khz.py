...
# Config
TARGET_SR = 44100  # Sample rate (44.1 kHz for full frequency range)
...
def prepare_file(source_path, target_path, target_duration, file_type):
    ...
    # Save as 24-bit
    sf.write(str(target_path), audio, TARGET_SR, subtype='PCM_24')
    print(f"\n  ✓ Saved to: {target_path} (24-bit, {TARGET_SR} Hz)")
...
if full_100 and stem_100:
    ...
    vocal_ok = prepare_file(stem_100, RTG_100_DIR / "isolated_vocal-44.wav", 4.7, "Isolated Vocal (100-window)")
    mixture_ok = prepare_file(full_100, RTG_100_DIR / "stereo_mixture-44.wav", 4.7, "Full Mixture (100-window)")
    ...
    if vocal_ok and mixture_ok:
        ...
        print(f"  {RTG_100_DIR / 'isolated_vocal-44.wav'}")
        print(f"  {RTG_100_DIR / 'stereo_mixture-44.wav'}")
        print("\nRun:")
    ...
    vocal_ok = prepare_file(stem_nl, RTG_NL_DIR / "isolated_vocal-44.wav", None, "Isolated Vocal (no-limit)")
    mixture_ok = prepare_file(full_nl, RTG_NL_DIR / "stereo_mixture-44.wav", None, "Full Mixture (no-limit)")
    ...
    if vocal_ok and mixture_ok:
        ...
        print(f"  {RTG_NL_DIR / 'isolated_vocal-44.wav'}")
        print(f"  {RTG_NL_DIR / 'stereo_mixture-44.wav'}")
        print("\nRun:")
